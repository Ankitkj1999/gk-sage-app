...
class _SignUpPageState extends State<SignUpPage> {
      ...
      if (userCredential != null) {
        await FirebaseService().checkUserExists(userCredential.user!.uid).then((bool userExist) async {
          if (!userExist) {
            ...
            _handleAfterSocialLogin(userCredential, _googleController);
          }
        });
      }
      ...
      if (userCredential != null) {
        await FirebaseService().checkUserExists(userCredential.user!.uid).then((bool userExist) async {
          if (!userExist) {
            ...
            _handleAfterSocialLogin(userCredential, _fbController);
          }
        });
      }
    }).onError((error, stackTrace) {
      ...
      if (userCredential != null) {
        await FirebaseService().checkUserExists(userCredential.user!.uid).then((bool userExist) async {
          if (!userExist) {
            ...
            _handleAfterSocialLogin(userCredential, _appleController);
          }
        });
      }
    }).onError((error, stackTrace) {
  ...
  _handleAfterSocialLogin(UserCredential userCredential, RoundedLoadingButtonController controller) async {
    ...
    if (userBloc.userData != null) {
      ...
      await sb.getSettingsData();
      await sb.getSpecialCategories();
      await ab.checkAds();
      controller.success();
      _gotoHomeScreen();
    } else {
      ...
      controller.reset();
      if (!mounted) return;
      openSnackbar(context, 'No user found');
    }
  }
